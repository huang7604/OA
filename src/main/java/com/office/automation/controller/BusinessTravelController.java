...
public class BusinessTravelController {
	...
	public String businessTravelForm(Model model){
		model.addAttribute("messageProcess", new MessageProcess());
		return "activity/businessTravelForm";
	}
	
	@RequestMapping("/businessTravelForm/startBusinessTravelProcess")
	public String addBusinessTravelProcess(Model model,MessageProcess messageProcess,HttpSession session){
		User user=(User)session.getAttribute("user");
		messageProcess.setUserName(user.getName());
		messageProcess.setProcessName("出差申请");
		...
		runtimeService.startProcessInstanceByKey("businessTravelProcessId", map);
		return "redirect:/home/listRunTask";
	}
	
	//完成任务
	@RequestMapping("/businessTravelForm/finishBusinessTravelTask/{id}")
	public String finishBusinessTravelTask(@PathVariable String id,@RequestParam String reason,@RequestParam String message,@RequestParam String user){
