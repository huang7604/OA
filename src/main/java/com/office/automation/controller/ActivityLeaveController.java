...
@Controller
@RequestMapping("/home")
public class ActivityLeaveController {
	@Autowired
	...
	public String leaveForm(Model model){
		...
		return "activity/leaveForm";
	}
	
	@RequestMapping("/leaveForm/startLeaveProcess")
	public String addLeaveProcess(Model model,MessageProcess messageProcess,HttpSession session){
		User user=(User)session.getAttribute("user");
		messageProcess.setUserName(user.getName());
		messageProcess.setProcessName("请假申请");
		...
		runtimeService.startProcessInstanceByKey("LeaveProcessId", map);
		return "redirect:/home/listRunTask";
	}
	
	//完成任务
	@RequestMapping("/leaveForm/finishLeaveTask/{id}")
	public String finishLeaveTask(@PathVariable String id,@RequestParam String reason,@RequestParam String message,@RequestParam String user){
