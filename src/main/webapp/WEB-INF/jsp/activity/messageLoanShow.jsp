...
<div>
<form action="/OfficeAutomation/home/loanForm/finishLoanTask/${id }" method="post">
       <table class="table table-striped table-bordered table-hover  table-condensed">
                ...
                <fmt:formatDate value="${message.cashierCreateDate }" var="cashierCreateDate" pattern="yyyy-MM-dd" />
               <tbody>
                    <tr>
                    ...
                    <tr>
                     ...
                     <tr>
                      <td>理由说明：</td>
                      <td colspan="3" align="center">${message.reason}${message.leaderName}</td>
                    </tr>
                    ...
                    <c:if test="${!empty message.managerName }">
                     ...
                     <c:if test="${!empty message.cashierName }">
                        <tr>
                         <td>出纳付款：</td>
                         <td colspan="3" align="center">
                           <c:if test="${empty message.cashierMessage }">
                             <div>
                               <select name="message" class="form-control">
                                 <option value="yes">同意</option>
                               </select>
                               <textarea class="form-control" placeholder="审批理由...." name="reason"></textarea>
                               <input type="hidden" name="user" value="cashier"></input>
                             </div>
                           </c:if>
                           <c:if test="${!empty message.cashierMessage }">
                             <div>
                               <div>
                                 <div class="message">${message.cashierMessage }</div>
                                 <div class="reason">${message.cashierReason }</div>
                               </div>
                               <div>
                                 <div class="name">${message.cashierName }</div>
                                 <div class="date">${cashierCreateDate }</div>
                               </div>
                             </div>
                           </c:if>
                        </td>
